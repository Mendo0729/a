...
# /var/log/secure (RHEL/CentOS) and /var/log/messages belong to group "adm" on
# Debian/Ubuntu and group "root" on RHEL/CentOS.  Adding the agent user to "adm"
# covers Debian; on RHEL we use POSIX ACLs which are always available (setfacl).
#
# We attempt both: usermod -aG adm is a no-op if the group does not exist.
# setfacl is tried next; if it is absent (rare) we fall back silently — the
# agent will still work, it just won't be able to read auth/syslog files.
usermod -aG adm "$AGENT_USER" 2>/dev/null || true

for logfile in /var/log/secure /var/log/messages /var/log/syslog /var/log/auth.log; do
  if [ -f "$logfile" ]; then
    if setfacl -m "u:${AGENT_USER}:r" "$logfile" 2>/dev/null; then
      ok "ACL granted: ${AGENT_USER} can read ${logfile}"
    fi
  fi
done

# Ensure logrotate preserves the ACL on newly rotated files by setting a
# default ACL on /var/log.  This is best-effort; failure is non-fatal.
setfacl -m "d:u:${AGENT_USER}:r" /var/log 2>/dev/null || true
